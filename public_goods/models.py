...
class Subsession(otree.models.BaseSubsession):
	...
	for p in self.get_players():
            p.treatment = self.session.session_type['treatment']
	...
	if self.round_number == Constants.starting_rounds[0]:
            order = Constants.mpcrOrders[self.session.session_type['mpcrOrder'] - 1]
            for k, v in order.items():
                if v == "vr": self.session.vars['varied_round'] = k
            print "Order {}: {}".format(self.session.session_type['mpcrOrder'], order)
...
class Player(otree.models.BasePlayer):
    ...
    def set_signal_value(self):
        ...
        vr = self.session.vars['varied_round']
        signalVariance = self.session.session_type['signalVariance']
    ...
    def get_signal_values(self):
        if self.session.session_type['signalVariance'] == 2:
