...
class PlayerBot(Bot):
    ...
    def play_round(self):
        yield (views.IDScreen, {'label': "joe"})
        yield (views.QuizIntro)
        yield (views.QuizFirst, self.get_answers(1))
        yield (views.QuizTwo, self.get_answers(2))
        yield (views.QuizThree, self.get_answers(3))
